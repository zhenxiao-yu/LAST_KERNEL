...
namespace Kamgam.UIToolkitTextAnimation
{
    ...
    public class UIBuilderWindowWrapper
    {
        ...
        public void Select(VisualElement element)
        {
            ...
            {
                Debug.LogWarning("The element is detached. Will abort.");
                return;
            }
            ...
            {
                Debug.LogWarning("Builder window, types, properties or methods not found. Aborting Select().");
            }
        }

       
        public void DebugCallMethod(string methodName, params object[] parameters)
        {
            if (BuilderWindow)
            {
                debugCallMethodInternal(BuilderWindow, methodName, parameters);
            }
        }

        private void debugCallMethodInternal(object instance, string methodName, object[] parameters)
        {
        ...
        public void DebugCallMethod(string fieldName, string methodName, params object[] parameters)
        {
            if (BuilderWindow)
            {
                var field = BuilderWindowType.GetMemberInfo(fieldName);
                if (field != null)
                {
                    var value = field.GetValue(BuilderWindow);
                    debugCallMethodInternal(value, methodName, parameters);
                }
            }
        }
    }
