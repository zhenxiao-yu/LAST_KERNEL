Shader "Markyu/FortStack/CustomPostProcess"
{
    Properties
    {
        ...
        // ── Pixel Art ───────────────────────────────────────────────────────────
        _PixelRes         ("Pixel Resolution (XY)", Vector)  = (320, 180, 0, 0)
        _PixelizeAmount   ("Pixelize Amount",   Range(0, 1)) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // ── 0. Pixelization — snap to 320×180 grid ────────────────────────
                // Nearest-neighbour downsample to the pixel-art resolution.
                // Overlay effects (vignette, scanlines, neon ring) stay smooth.
                float2 pixelUV  = floor(i.uv * _PixelRes.xy) / _PixelRes.xy;
                float2 sampleUV = lerp(i.uv, pixelUV, _PixelizeAmount);

                float2 centered = i.uv - 0.5;
                float  radDist  = length(centered);

                // ── 1. Chromatic Aberration (sampled from pixelated UV) ───────────
                float2 aberrDir = normalize(centered + 0.0001) * _ChromaticAmount;
                float r = tex2D(_MainTex, sampleUV + aberrDir).r;
                float g = tex2D(_MainTex, sampleUV).g;
                float b = tex2D(_MainTex, sampleUV - aberrDir).b;
